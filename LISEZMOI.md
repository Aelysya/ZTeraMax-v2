...
Tout d'abord, il est fortement recommandé d'utiliser un datapack que vous pouvez trouver [ici](https://github.com/PokemonWorkshop/GameDataPacks/tree/main). De nombreuses données utilisées dans ce plugin sont erronées dans les données de base d'un projet Studio, donc la plupart des fonctionnalités ne fonctionneront tout simplement pas.
...
Vous avez peut-être une UI de combat personnalisée pour votre fangame, et si c'est le cas, vous avez peut-être changé les boutons de sélection des capacités. Par défaut, ce plugin raccourcit les noms des capacités pour qu'ils ne dépassent pas 15 caractères. Ceci afin d'éviter que les noms des capacités Z, qui sont parfois trèèès longs, ne sortent de la fenêtre. Si vous avez déjà un moyen de gérer ce genre de problème ou si le raccourcissement n'est pas nécessaire pour votre jeu, vous pouvez aller dans le fichier de configuration `Data/configs/z_tera_max_config.json` et modifier la valeur de `useBuiltinMoveNameSlice` en false.
...
Pour qu'un dresseur IA puisse utiliser le Dynamax, vous devez rajouter une entrée dans la liste du champ `dynamaxMarkers` du fichier de configuration. Les champ internes ont un nom assez évident et vous ne devriez pas avoir trop de souci pour les remplir. Dès que le dresseur envoi un Pokémon avec un db_symbol correspondant à l'un de ses marqueurs, il le fera Dynamaxer et pourra utiliser des capacités Max.
...
- Pour le fichier `types_BATTLE. png`, même principe que l'instruction précédente, il fait juste référence au fichier `types.png` situé dans le dossier `graphics/interface/battle`, pensez-bien à le renommer en `types.png` après l'avoir copié
...
Pour qu'un dresseur IA puisse utiliser le Téracristal, vous devez rajouter une entrée dans la liste du champ `terastalMarkers` du fichier de configuration. Les champ internes ont un nom assez évident et vous ne devriez pas avoir trop de souci pour les remplir. Dès que le dresseur envoi un Pokémon avec un db_symbol correspondant à l'un de ses marqueurs, il le fera Téracristalliser.
